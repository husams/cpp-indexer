...
const FactQuery kFactQueries[] = {
    ...
    {"symbol_kind", 2, "SELECT id, name FROM symbol_kind ORDER BY id"},
    {"edge", 5,
     "SELECT id, src_id, dst_id, kind, count FROM edge ORDER BY id"},
    {"edge_kind", 2, "SELECT id, name FROM edge_kind ORDER BY id"},
    {"edge_site", 4,
     "SELECT edge_id, file_id, COALESCE(line, 0), COALESCE(col, 0)"
     " FROM edge_site ORDER BY edge_id, file_id, line, col"},
    {"entity_node", 2, "SELECT id, kind FROM entity_node ORDER BY id"},
    {"entity_kind", 2, "SELECT id, name FROM entity_kind ORDER BY id"},
    {"entity_edge", 6,
...
std::vector<std::string> output_relations(const std::string &program) {
  ...
  while (std::getline(iss, line)) {
    ...
    while (name_end < line.size() && ident(line[name_end], name_end == name_begin))
      ++name_end;
...
int cmd_analyze(const ParsedArgs &args, Context &ctx) {
  ...
  if (args.analyze_list) {
    ...
    for (const BuiltinRule &rule : kBuiltinRules) {
      json_out::Object obj;
      obj.push_back({"name", json_out::Value::of(std::string(rule.name))});
      obj.push_back(
          {"description", json_out::Value::of(std::string(rule.description))});
      rules.push_back(json_out::Value::obj(std::move(obj)));
    }
    json_out::Object top;
    top.push_back({"rules", json_out::Value::arr(std::move(rules))});
  ...
  try {
    ...
    for (const auto &[name, rows] : relations) {
      ...
      rel_obj.push_back({name, json_out::Value::arr(std::move(rows_arr))});
    }
    json_out::Object top;
    top.push_back({"rule", json_out::Value::of(label)});
    top.push_back({"db", json_out::Value::of(pathutil::abspath(ctx.index_path))});
    top.push_back({"relations", json_out::Value::obj(std::move(rel_obj))});
    *ctx.out << json_out::dumps_indent2(json_out::Value::obj(std::move(top)))
             << "\n";
    return 0;
