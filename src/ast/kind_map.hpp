...
// 17-entry map, storage.cpp:463-485). The Clang C++ API exposes
// clang::Decl::Kind instead, so this translates one to the other, keeping the
// on-disk schema byte-identical. Returns -1 for decls cidx does not persist.
#pragma once
...
// Storage CXCursorKind integer for a kind name, or -1 when unmapped.
