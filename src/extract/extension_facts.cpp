...
auto node_key(const ExtensionNodeFact &fact) {
  return std::tie(fact.provenance.plan_hash, fact.provenance.rule_id,
                  fact.namespace_name, fact.node_kind, fact.identity,
                  fact.evidence.file, fact.evidence.line, fact.evidence.col);
}

auto relation_key(const ExtensionRelationFact &fact) {
  return std::tie(fact.provenance.plan_hash, fact.provenance.rule_id,
                  fact.namespace_name, fact.relation_kind, fact.from_identity,
                  fact.to_identity, fact.evidence.file, fact.evidence.line,
                  fact.evidence.col);
}

auto attribute_key(const ExtensionAttributeFact &fact) {
  return std::tie(fact.provenance.plan_hash, fact.provenance.rule_id,
                  fact.namespace_name, fact.attribute_name, fact.identity,
                  fact.value, fact.evidence.file, fact.evidence.line,
                  fact.evidence.col);
}

auto unknown_key(const ExtensionUnknownFact &fact) {
  return std::tie(fact.provenance.plan_hash, fact.provenance.rule_id,
                  fact.namespace_name, fact.reason_code, fact.identity,
                  fact.evidence.file, fact.evidence.line, fact.evidence.col);
}
...
std::string InMemoryExtensionFactSink::canonical_text() const {
  ...
  for (const auto &fact : nodes_) {
    out << "node\t" << fact.provenance.plan_hash << '\t'
        << fact.provenance.rule_id << '\t' << fact.provenance.producer_package
