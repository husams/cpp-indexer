...
def load() -> tuple[dict[str, Any], str]:
    ...
    if not isinstance(views, list) or not views:
        raise SystemExit("views must be a non-empty array")
    if len(views) != len(set(views)) or any(not isinstance(view, str) or not view for view in views):
        raise SystemExit("views must contain unique non-empty strings")
    ...
    for row in data["relations"]:
        ...
        if row["layer"] not in views:
            raise SystemExit(f"relation {row['name']} uses unknown layer {row['layer']!r}")
...
def render_cpp(data: dict[str, Any], digest: str) -> str:
    ...
    def relation_row(item: dict[str, Any]) -> str:
        values = [
            ("id", str(item["id"])),
            ("name", json.dumps(item["name"])),
            ("layer", "View::" + py_name(item["layer"])),
            ("source", json.dumps(item.get("source", item["layer"]))),
            ("target", json.dumps(item.get("target", item["layer"]))),
            ("inverse", json.dumps(item["inverse"])),
            ("traversal", json.dumps("|".join(item["traversal"]))),
            ("evidence", json.dumps(item["evidence"])),
            ("evidence_capabilities", json.dumps("|".join(item["evidence_capabilities"]))),
            ("completeness", json.dumps(item["completeness"])),
            ("virtual_relation", str(item.get("virtual", False)).lower()),
        ]
        return (
            ...
            + "\n".join(
                ...
                for index, (name, value) in enumerate(values[1:], start=1)
            )
            + "\n"
        )
    ...
    relations = "".join(relation_row(item) for item in data["relations"])
    fields = "".join(
        row(
    ...
    lines = [
        ...
        f"inline constexpr std::string_view kCatalogHash =\n    {json.dumps(digest)};",
        "enum class View : std::uint8_t {",
        *[
            f"  {py_name(view)}{',' if index < len(data['views']) - 1 else ''}"
        ...
        "  std::string_view traversal;",
        "  std::string_view evidence;",
        "  std::string_view evidence_capabilities;",
        "  std::string_view completeness;",
...
def render_sql(data: dict[str, Any], digest: str) -> str:
    ...
    for table, rows in (("symbol_kind", data["symbol_kinds"]), ("edge_kind", [r for r in data["relations"] if r["layer"] == "symbol" and not r.get("virtual", False)]), ("entity_edge_kind", [r for r in data["relations"] if r["layer"] == "entity" and not r.get("virtual", False)]), ("entity_kind", data["entity_kinds"]), ("type_kind", data["type_kinds"]), ("type_edge_kind", data["type_edge_kinds"])):
    ...
    for relation in data["relations"]:
        ...
        value = canonical({k: relation[k] for k in ("name", "layer", "source", "target", "inverse", "traversal", "evidence", "evidence_capabilities", "completeness", "virtual") if k in relation})
...
def render_extensions_cpp(data: dict[str, Any], digest: str) -> str:
    ...
    for extension in data.get("extensions", []):
        for relation in extension["relations"]:
            ...
            fields = [f".{name} = {json.dumps(value)}" for name, value in zip(names, values)]
            rows.append(
                "    {" + fields[0] + ",\n"
                + "\n".join(
                    "     " + field + ("," if index < len(fields) - 1 else "},")
    ...
    return (
        ...
        "  std::string_view completeness;\n"
        "};\n"
        f"inline constexpr std::string_view kExtensionCatalogHash =\n    {json.dumps(digest)};\n"
