...
def format_generated_cpp(source: str) -> str:
    candidates = [
        "clang-format",
        "clang-format-22",
        "clang-format-21",
    ...
    if formatter is None:
        raise SystemExit(
            "catalog generation requires clang-format (LLVM 18+); "
            "install the LLVM clang tools"
        )
    ...
    if result.returncode != 0:
        raise SystemExit(
            f"clang-format failed while formatting generated catalog C++: "
...
def render_cpp(data: dict[str, Any], digest: str) -> str:
    ...
    relations = "".join(
        row(
            ["id", "name", "layer", "source", "target", "inverse", "traversal", "evidence", "evidence_capabilities", "completeness", "virtual_relation"],
            [
                str(item["id"]),
                json.dumps(item["name"]),
                "View::" + py_name(item["layer"]),
                ...
                str(item.get("virtual", False)).lower(),
            ],
        )
    ...
    lines = [
        ...
        "enum class View : std::uint8_t { " + ", ".join(py_name(view) for view in data["views"]) + " };",
...
def render_sql_header(data: dict[str, Any], digest: str) -> str:
    ...
    source = f'''// Generated by scripts/generate_catalogs.py; DO NOT EDIT.
#pragma once
#include <string_view>
...
'''
    return format_generated_cpp(source)
