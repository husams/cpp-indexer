...
def load() -> dict:
    ...
    for key in ("statuses", "exit_classes", "freshness", "completeness_states", "diagnostic_severities", "evidence_classes", "trust_levels", "event_kinds", "diagnostic_codes", "artifact_kinds", "artifact_generation", "placeholder_identities", "diagnostic_invariants", "diagnostic_status_invariants", "acceptance_vectors", "adversarial_vectors"):
        if not data.get(key):
            raise ValueError(f"protocol source requires {key}")
    artifact_generation = data["artifact_generation"]
    if (
        artifact_generation.get("field") != "generation"
        or artifact_generation.get("required") is not False
        or artifact_generation.get("min_length") != 1
    ):
        raise ValueError("artifact generation must be optional and nonempty")
...
def render_typescript(data: dict) -> str:
    limits = data["limits"]
    exits = {item["name"]: item["code"] for item in data["exit_classes"]}
    generation_field = data["artifact_generation"]["field"]
    return f'''// Generated by scripts/generate_result_protocol.py; DO NOT EDIT.
// Source: spec/contracts/result-protocol.json

export const PROTOCOL = "cidx.result/v1" as const;
export const PROTOCOL_VERSION = {data["protocol_version"]} as const;
export const EXIT_CODES = {json.dumps(exits, separators=(",", ":"))} as const;
export const MAX_TEXT_BYTES = {limits["max_text_bytes"]} as const;
export const MAX_RESULT_DEPTH = {limits["max_result_depth"]} as const;
export const MAX_RESULT_PROPERTIES = {limits["max_result_properties"]} as const;
export const MAX_RESULT_ITEMS = {limits["max_result_items"]} as const;
export const MAX_DIAGNOSTICS = {limits["max_diagnostics"]} as const;
export const MAX_EVIDENCE = {limits["max_evidence"]} as const;
export const MAX_ARTIFACTS = {limits["max_artifacts"]} as const;
export const MAX_FACT_SETS = {limits["max_fact_sets"]} as const;
export const MAX_REPLAY_ARGUMENTS = {limits["max_replay_arguments"]} as const;

export type ResultStatus = {ts_union(data["statuses"])};
export type ExitClass = {ts_union([item["name"] for item in data["exit_classes"]])};
export type Freshness = {ts_union(data["freshness"])};
export type CompletenessState = {ts_union(data["completeness_states"])};
export type DiagnosticSeverity = {ts_union(data["diagnostic_severities"])};
export type DiagnosticCode = {ts_union(data["diagnostic_codes"])};
export type EvidenceClass = {ts_union(data["evidence_classes"])};
export type TrustLevel = {ts_union(data["trust_levels"])};
export type ArtifactKind = {ts_union(data["artifact_kinds"])};
...
def render_schema(data: dict) -> dict:
    ...
    artifact_generation = data["artifact_generation"]
    artifact_properties = {
        "kind": {"enum": data["artifact_kinds"]},
        "id": text,
        "schema_version": {
            "type": "integer",
            "minimum": 1,
            "maximum": numeric["max_integer"],
        },
        "catalog_version": {
            "type": "integer",
            "minimum": 1,
        ...
        "catalog_hash": text,
        artifact_generation["field"]: {
            **text,
            "minLength": artifact_generation["min_length"],
        },
    }
    schema = {
        ...
        "properties": {
            ...
            "artifacts": {"type": "array", "maxItems": limits["max_artifacts"], "items": {"type": "object", "required": ["kind", "id", "schema_version", "catalog_version", "catalog_hash"], "additionalProperties": False, "properties": artifact_properties}},
...
def main() -> int:
    ...
    try:
        ...
        outputs = {
            ...
            ROOT / "ui/src/generated/result_protocol.ts": render_typescript(data),
