...
#: ``cidx cache pch build --include <header>``.
...
# -- corpus header survey (cidx cache pch build --from-corpus) -----------------------
...
def _umbrella_text(headers: Sequence[str], quoted: bool = False) -> str:
    lines = [
        "// Generated by `cidx cache pch build` -- shared system/C++ precompiled header.",
        "// Edit via `cidx cache pch build --include <header>`; do not hand-edit.",
    ]
...
def cmd_build(
    ...
    """``cidx cache pch build``: derive the common C++ flags from the index, compile
...
def cmd_status() -> int:
    """``cidx cache pch status``: show the cached PCH's size, flags, and validity."""
    pp = pch_path()
    if not os.path.exists(pp):
        print("no system PCH built (run `cidx cache pch build`)")
        return 0
...
def cmd_clear() -> int:
    """``cidx cache pch clear``: remove the cached PCH, its sidecar, and umbrella."""
    removed = 0
    freed = 0
