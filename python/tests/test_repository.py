...
def test_fresh_schema_has_repository_clone_and_column():
    ...
    try:
        ...
        assert ver == str(SCHEMA_VERSION) == "39"
        tables = {
            r[0]
...
def test_cli_import_groups_and_switch_rebases(tmp_path):
    ...
    rs = _cidx(cache, "repo", "show", repo_name)
    ...
    lc_db = _cidx(cache, "repo", "show", repo_name)
    ...
    wt = tmp_path / "wt"
    wt.mkdir()
    ac = _cidx(cache, "repo", "add-clone", repo_name, str(wt), "--label", "feat")
    assert ac.returncode == 0, ac.stderr
    sw = _cidx(cache, "repo", "switch", repo_name, "feat")
    ...
    sw2 = _cidx(cache, "repo", "switch", repo_name, orig)
