...
records the frontend and Cytoscape versions, catalog/schema references, and the
pinned `cpp-indexer` and `banking` workspace identities. The browser limits are
published in [performance-budget.json](../web/performance-budget.json), and the
...
budget refusal, security-safe rendering of hostile labels and locations, DOM
keyboard alternatives, continuation/staleness behavior, and the offline
`file://` smoke test with zero network requests.
...
workspace fixture identity is checked by `npm run qualify`; semantic corpus
drift remains visible through the GraphView identity and result status rather
