...
change), and the written explicit-specifier including an `explicit(false)` or
dependent `explicit(<expr>)` condition (which `isExplicit()` alone collapses to
"not explicit"). The semantic API-state comparison, by contrast, compares
effective state.
