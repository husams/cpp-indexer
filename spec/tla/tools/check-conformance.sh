...
# a protected invariant or a conformance mapping could otherwise replace this
# script with a stub that exits 0 in the same commit. CIDX_REPO_ROOT lets
# that extracted copy -- which no longer lives inside the repository tree --
# still resolve ROOT to the real checkout's spec/tla/ (this PR's head), so it
# replays the PR's actual conformance data, not files relative to its own
